...
This project has code intelligence tools available via MCP (`kb_callers`, `kb_symbols`, `kb_code_doctor`, `kb_code_query`, etc.) and CLI (`git-kb code callers`, `git-kb code doctor`, `git-kb code query`, etc.). **Prefer MCP tools** — they support parallel calls and return structured JSON. Fall back to CLI via Bash if MCP is disconnected.
